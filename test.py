...
# Define global lists for known encodings and class names
encodeListKnown = []
classNames = []
...
def image_fetch():
    ...
    try:
        with open('.venv/encryption_key.key', 'rb') as key_file:
            key = key_file.read()
    ...
    try:
        with open('.venv/encodings_list_encrypted.pkl', 'rb') as file:
    ...
    try:
        with open('.venv/class_names_encrypted.pkl', 'rb') as file:
